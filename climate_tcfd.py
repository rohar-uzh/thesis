"""
ClimateBERT TCFD Pillar Classifier
------------------------------------
...
# Fallback label map — used only if model.config.id2label is absent.
...
def load_classifier():
    """
    Load the ClimateBERT TCFD pillar classifier.

    Returns
    -------
    clf : HuggingFace text-classification pipeline
    id2label : dict mapping raw labels to human-readable pillar names
    """
...
def load_and_validate_data(filepath: str) -> pd.DataFrame:
    ...
    n_climate = (df["detector_label"] == DETECTOR_POSITIVE).sum()
    n_total = len(df)
...
def run_inference(clf, texts: list, batch_size: int = BATCH_SIZE) -> list:
    """
    Run the TCFD pillar classifier on a list of texts in batches.
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Classifying TCFD pillars"):
...
def generate_output_filename(input_path: str) -> str:
    ...
    Example:
        ...
        output: results/tcfd/UBS_2023_Annual_tcfd_2026-04-12.xlsx
    """
    base = os.path.splitext(os.path.basename(input_path))[0]

    # Strip any existing stage suffix
    for suffix in ("_detected", "_specificity", "_commitment", "_tcfd"):
        if suffix in base:
    ...
    output_dir = os.path.join("results", "tcfd")
    os.makedirs(output_dir, exist_ok=True)

    return os.path.join(output_dir, f"{base}_tcfd_{today}.xlsx")
...
def run_tcfd_classification(
    ...
    """
    Full TCFD pipeline: load → classify climate rows → save → return.

    Only paragraphs with detector_label == 'yes' are classified.
    All other rows receive NaN for TCFD columns.
    ...
    (df, output_path) : tuple
        df          — Full DataFrame with added 'tcfd_label' and 'tcfd_score'
    ...
    df.loc[climate_mask, "tcfd_label"] = [r["label"] for r in results]
    df.loc[climate_mask, "tcfd_score"] = [round(r["score"], 4) for r in results]

    # ── TCFD summary ──────────────────────────────────────────────────────────
    climate_df = df[climate_mask]
    counts = climate_df["tcfd_label"].value_counts()
    ...
    for label in id2label.values():
        ...
        print(f"  {label:<22}: {count:>6,}  ({count / n_climate * 100:.1f}%)")
    print()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python climate_tcfd.py <input.xlsx> [output.xlsx]")
        sys.exit(1)

    in_path = sys.argv[1]
    out_path = sys.argv[2] if len(sys.argv) > 2 else None
    run_tcfd_classification(in_path, out_path)