"""
ClimateBERT Climate Commitment Classifier
-----------------------------------------
Classifies climate-related paragraphs as commitment/action or non-commitment
using the ClimateBERT distilroberta-base-climate-commitment model.
...
Usage (from Colab notebook):
    from climate_commitment import run_commitment_classification
    df, output_file = run_commitment_classification("data/detected/UBS_2023_Annual_detected_2026-04-12.xlsx")
"""
...
def load_classifier():
    """
    Load the ClimateBERT commitment model.
...
def run_inference(clf, texts: list, batch_size: int = BATCH_SIZE) -> list:
    """
    Run the commitment classifier on a list of texts in batches.
    ...
    for i in tqdm(range(0, len(texts), batch_size), desc="Classifying commitment"):
...
def generate_output_filename(input_path: str) -> str:
    ...
    Example:
        ...
        output: results/commitment/UBS_2023_Annual_commitment_2026-04-12.xlsx
    """
    base = os.path.splitext(os.path.basename(input_path))[0]

    # Strip any existing stage suffix (_detected, _specificity, _commitment)
    ...
    output_dir = os.path.join("results", "commitment")
    os.makedirs(output_dir, exist_ok=True)

    return os.path.join(output_dir, f"{base}_commitment_{today}.xlsx")


# ── Main entry point ──────────────────────────────────────────────────────────

def run_commitment_classification(
    input_path: str,
    output_path: str = None,
    batch_size: int = BATCH_SIZE,
) -> tuple:
    """
    Full commitment pipeline: load → classify climate rows → save → return.

    Only paragraphs with detector_label == 'yes' are classified.
    All other rows receive NaN for commitment columns.
    ...
    (df, output_path) : tuple
        df          — Full DataFrame with added 'commitment_label' and 'commitment_score'
    ...
    df["commitment_label"] = None
    df["commitment_score"] = None
    ...
    df.loc[climate_mask, "commitment_label"] = [r["label"] for r in results]
    df.loc[climate_mask, "commitment_score"] = [round(r["score"], 4) for r in results]

    # ── Commitment summary ────────────────────────────────────────────────────
    climate_df = df[climate_mask]
    counts = climate_df["commitment_label"].value_counts()
    n_climate = len(climate_df)

    print("\n--- Commitment Summary (climate paragraphs only) ---")
    print(f"Label mapping: {id2label}")
    print()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python climate_commitment.py <input.xlsx> [output.xlsx]")
        sys.exit(1)

    in_path = sys.argv[1]
    out_path = sys.argv[2] if len(sys.argv) > 2 else None
    run_commitment_classification(in_path, out_path)
